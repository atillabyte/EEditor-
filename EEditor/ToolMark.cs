        ...
        public void PlaceBorderRect()
        {
            ...
            for (int x = 0; x < r.Width; ++x)
                for (int y = 0; y < r.Height; ++y)
                {
                    if (0 <= x + r.X && x + r.X < editArea.BlockWidth && 0 <= y + r.Y && y + r.Y < editArea.BlockHeight)
                    {
                        editArea.Draw(x + r.X, y + r.Y, g, Convert.ToInt32(Back[y, x]), Convert.ToInt32(Front[y, x]), Convert.ToInt32(Coins[y, x]), Convert.ToInt32(Id1[y, x]), Convert.ToInt32(Target1[y, x]), Text1[y, x], MainForm.userdata.thisColor);
                    }
            ...
            if (Front != null)
            {
                ...
                for (int y = 0; y < Rect.Height; ++y)
                {
                    for (int x = 0; x < Rect.Width; ++x)
                    {
                        ...
                        if (0 <= xx && xx < editArea.BlockWidth && 0 <= yy && yy < editArea.BlockHeight)
                        {
                            if (IsPaintable(xx, yy, Convert.ToInt32(Front[y, x]), true) && IsPaintable(xx, yy, Convert.ToInt32(Front[y, x]), false))
                            {
                                frame.Foreground[yy, xx] = Convert.ToInt32(Front[y, x]);
                            }
                            else if (!IsPaintable(xx, yy, Convert.ToInt32(Front[y, x]), true))
                            {
                                frame.Foreground[yy, xx] =  9;
                            }

                            if (Convert.ToInt32(Back[y, x]) != 0) frame.Background[yy, xx] = Convert.ToInt32(Back[y, x]);
                            else frame.Background[yy, xx] = 0;
                            frame.BlockData[yy, xx] = Convert.ToInt32(Coins[y, x]);
                            frame.BlockData1[yy, xx] = Convert.ToInt32(Id1[y, x]);
                            frame.BlockData2[yy, xx] = Convert.ToInt32(Target1[y, x]);
                            frame.BlockData3[yy, xx] = Text1[y, x];
                            editArea.Draw(xx, yy, g, MainForm.userdata.thisColor);
                            //g.DrawImage(editArea.Bricks[frame.Map[yy, xx]], (x + Rect.X) * 16, (y + Rect.Y) * 16);
                        }
                    }
